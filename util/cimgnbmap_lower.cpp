 ...
 * Based on cimgnbmap from Stella X. Yu, Nov 12, 2001.
 * function [i,j] = cimgnbmap_lower([nr,nc], nb_r, sample_rate)
...
)
{
    ...
    p = (unsigned int*)mxCalloc(np * (nb+1), sizeof(unsigned int));
    if (p==NULL) {
